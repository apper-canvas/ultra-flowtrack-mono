...
const handleToggleComplete = () => {
    onUpdate(task.Id, {
      ...
      completedAt_c: task.status_c === "completed" ? null : new Date().toISOString()
    })
  }
...
<div className="text-xs text-slate-500 space-y-1">
          <div>Created {format(new Date(task.createdAt_c), "MMM dd, yyyy")}</div>
          {isCompleted && task.completedAt_c && (
            <div className="text-success-600">
              Completed {format(new Date(task.completedAt_c), "MMM dd, yyyy")}
            </div>
          )}
        </div>
